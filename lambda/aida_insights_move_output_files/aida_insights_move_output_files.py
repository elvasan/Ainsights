...
def lambda_handler(event, context):
    ...
    try:
        ...
        external_output_file_key = s3.list_objects(
            Bucket=bucket_name,
            Prefix=external_output_file_prefix
        )['Contents'][0]['Key']

        # Download the output file to /tmp/
        s3.download_file(bucket_name, external_output_file_key, '/tmp/'+external_file_name)
        ...
        s3.upload_file('/tmp/'+zip_file_name, bucket_name, 'outgoing/'+client_name+'/'+zip_file_name)
